...
# IMPORTANT NOTES:
# Set the environment variable VLLM_USE_V1 to 0
# You have to use v0, so set the env var VLLM_USE_V1=0
# It works in v1 too but you have to do some rpc stuff to get the subprocess to register the hook
# it works for us with vllm==0.10.0

# this is a throwaway script I used for testing patchscopes style steering with vllm
# I think you need to set the following LLM vars when loading the model:
# disable_async_output_proc=True,
# enforce_eager=True,
# enable_prefix_caching=False,

# applying steering at every token position is very easy
# you can also look at our implementation of collecting activations
...
# with the continual batching, which would probably be easiest with some global variables to keep track of which prompts have been seen so far and then line it up with the position ids you currently are seeing in the hook
...
def run_classification(tokenizer: AutoTokenizer, llm: LLM, LAYER: int, dataset_name: str, max_examples: int = 500, act_layer: int = 9):
...
for act_layer in act_layers:
    ...
    results = run_classification(tokenizer, llm, LAYER, dataset_name, max_examples, act_layer)
    all_results.append(results)
